...
export default function Home() {
  ...
  const rankedMissions = useMemo(() => rankMissions(state), [state]);
  const mainMission = activeMission ?? recommendation?.mission;
  const alternatives = rankedMissions.filter((item) => item.mission.id !== mainMission?.id).map((item) => item.mission);
  ...
  const completeMission = () => {
    ...
    setState((current) => ({
      ...
      player: { ...current.player, focusMinutes: current.player.focusMinutes + activeMission.minutes, momentum: Math.min(100, current.player.momentum + 4) },
    }));
    setProof("");
  };
  ...
  const addMissionList = () => {
    ...
    const engine = state.engines.find((item) => item.id === bulkEngine);
    const kind: NonNullable<Mission["kind"]> = engine?.status === "experiment" ? "validate" : "improve";
    setState((current) => ({ ...current, missions: [...current.missions, ...titles.map((title) => ({ id: crypto.randomUUID(), title, engineId: bulkEngine, whyNow: "Додано до черги; уточни причину пріоритету перед стартом.", minutes: 45, status: "planned" as const, kind, leverage: 3, priority: "normal" as const }))] }));
    setBulkText("");
  };
  ...
  const editMission = (missionId: string) => {
    ...
    const kind = window.prompt("Тип: earn / validate / build / improve", mission.kind ?? "improve");
    if (!kind || !["earn", "validate", "build", "improve"].includes(kind)) return;
    const leverage = Number(window.prompt("Важіль від 1 до 5", String(mission.leverage ?? 3)));
    if (!Number.isInteger(leverage) || leverage < 1 || leverage > 5) return;
    const priority = window.prompt("Пріоритет: primary / secondary / normal", mission.priority ?? "normal");
    if (!priority || !["primary", "secondary", "normal"].includes(priority)) return;
    setState((current) => ({ ...current, missions: current.missions.map((item) => item.id === missionId ? { ...item, title: title.trim(), whyNow: whyNow.trim(), minutes, kind: kind as Mission["kind"], leverage, priority: priority as Mission["priority"] } : item) }));
  };
  ...
  const createMission = () => {
    ...
    if (!Number.isFinite(minutes) || minutes < 30 || minutes > 90) return;
    const selectedEngine = state.engines.find((item) => item.id === engineId);
    setState((current) => ({ ...current, missions: [...current.missions, { id: crypto.randomUUID(), title: title.trim(), engineId, whyNow: whyNow.trim(), minutes, status: "planned", kind: selectedEngine?.status === "experiment" ? "validate" : "improve", leverage: 3, priority: "normal" }] }));
  };
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <section className="rounded-3xl border border-cyan-300/30 bg-gradient-to-br from-cyan-400/10 to-indigo-400/10 p-6 shadow-2xl shadow-cyan-950/20 sm:p-8">
          ...
          {mainMission && engine ? <>
            ...
            {!activeMission && recommendation && <p className="mt-3 text-sm text-cyan-100">Вибрано Decision Engine: {recommendation.reasons.join(" · ")}</p>}
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.4fr_1fr]">
          <section>
            <div className="mb-4 flex items-center justify-between gap-4"><div><h2 className="mb-1 text-lg font-semibold">Альтернативи</h2><p className="text-sm text-slate-500">Лише дві найближчі. Повний список — окремо.</p></div><Link href="/missions" className="text-sm text-cyan-700 underline underline-offset-4">Відкрити чергу</Link></div><div className="space-y-3">{alternatives.slice(0, 2).map((mission, index) => { const alternativeEngine = state.engines.find((item) => item.id === mission.engineId); return <div key={mission.id} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white/70 p-4"><span className="w-6 text-center text-sm font-semibold text-cyan-700">{index + 2}</span><button onClick={() => chooseMission(mission)} disabled={Boolean(activeMission)} className="flex-1 text-left disabled:opacity-50"><p className="text-sm text-cyan-700">{alternativeEngine?.name} · {mission.minutes} хв</p><p className="mt-1 font-medium leading-6">{mission.title}</p></button></div>; })}</div>
          ...
          <aside className="space-y-6"><section className="rounded-2xl border border-rose-400/20 bg-rose-400/5 p-5"><h2 className="font-semibold text-rose-200">Сьогодні не робимо</h2><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-300">{state.todayBoundary.map((item) => <li key={item}>— {item}</li>)}</ul></section><Link href="/state" className="block rounded-2xl border border-slate-700 bg-slate-900/60 p-5"><h2 className="font-semibold">Твій стан</h2><p className="mt-3 text-sm text-slate-300">Momentum: {state.player.momentum}/100</p><p className="mt-1 text-sm text-slate-400">Фокус: {state.player.focusMinutes}/{state.player.capacityMinutes} хв</p>{state.player.currentBoss && <p className="mt-3 text-sm text-amber-200">Активний бос — відкрий стан</p>}</Link><section className="rounded-2xl border border-slate-700 bg-slate-900/60 p-5"><h2 className="font-semibold">Останній доказ</h2><p className="mt-3 text-sm leading-6 text-slate-400">{lastProof ?? "Ще немає. Заверши одну місію з коротким фактом."}</p></section></aside>
        </div>
      </div>
    </main>
  );
