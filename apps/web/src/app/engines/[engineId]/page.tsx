...
export default function EnginePage() {
  ...
  return <main className="min-h-screen px-5 py-10 sm:px-8"><div className="mx-auto max-w-3xl">
    ...
    <section className="mt-5 rounded-3xl border p-6"><h2 className="text-lg font-semibold text-slate-900">Поточний стан</h2><div className="mt-5 grid gap-4"><label className="text-sm font-medium text-slate-700">Статус<select value={engine.status} onChange={(event) => updateEngine({ status: event.target.value as Engine["status"] })} className="mt-2 w-full rounded-xl border p-3 text-slate-800">{Object.entries(statusLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>{engine.status === "experiment" && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-medium text-amber-900">Лабораторія: {state.experimentFocusId === engine.id ? "активний експеримент" : "неактивна"}</p><p className="mt-1 text-sm text-amber-800">Decision Engine дає місії лише одному експерименту одночасно — і лише для перевірки попиту.</p><button onClick={() => setState((current) => ({ ...current, experimentFocusId: engine.id }))} className="mt-3 rounded-lg border border-amber-300 px-3 py-2 text-sm font-semibold text-amber-900">Зробити активною лабораторією</button></div>}<label className="text-sm font-medium text-slate-700">Ціль доходу на місяць, $<input type="number" min="0" value={engine.targetMonthly ?? ""} onChange={(event) => updateEngine({ targetMonthly: event.target.value === "" ? undefined : Number(event.target.value) })} className="mt-2 w-full rounded-xl border p-3 text-slate-800" /></label><label className="text-sm font-medium text-slate-700">Стадія<textarea value={engine.stage} onChange={(event) => updateEngine({ stage: event.target.value })} className="mt-2 min-h-20 w-full rounded-xl border p-3 text-slate-800" /></label><label className="text-sm font-medium text-slate-700">Одне вузьке місце<textarea value={engine.bottleneck} onChange={(event) => updateEngine({ bottleneck: event.target.value })} className="mt-2 min-h-20 w-full rounded-xl border p-3 text-slate-800" /></label><label className="text-sm font-medium text-slate-700">Критерій переходу далі<textarea value={engine.objective} onChange={(event) => updateEngine({ objective: event.target.value })} className="mt-2 min-h-20 w-full rounded-xl border p-3 text-slate-800" /></label></div></section>
