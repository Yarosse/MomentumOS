...
import { normalizeMomentumState, storageKey, type MomentumState } from "@/lib/momentum";
...
export default function LogPage() {
  ...
  const save = () => { const logs = JSON.parse(localStorage.getItem(key) ?? "[]") as Log[]; const entry = { date: new Date().toDateString(), done, blocker, insight, energy }; const next = logs.filter((item) => item.date !== entry.date).concat(entry); localStorage.setItem(key, JSON.stringify(next)); const raw = localStorage.getItem(storageKey); if (raw) { const momentum = normalizeMomentumState(JSON.parse(raw) as MomentumState); localStorage.setItem(storageKey, JSON.stringify({ ...momentum, player: { ...momentum.player, energy } })); } setSaved(true); };
